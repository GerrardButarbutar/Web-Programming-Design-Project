...
<html lang="en">
  <head>
    ...
    <title>MEMBER LIST - Jordan Petshop</title>
  </head>
  <body>
 ...
 <div class="d-flex flex-column p-3 text-white bg-dark" style="width: 280px; height: 800px;">
  ...
  <div class="container-l me-5">
    <h2>MEMBER LIST</h2>
    <div class="button text-center text-light border bg-success mt-3" style="border-radius: 5%; width: 110px; height: 30px;">New Member</div>
  <table class="table mt-3">
    ...
    if(mysqli_num_rows($rs) > 0){
        while($row= $rs->fetch_assoc()){
            $ptrid = $row['id'];
            $ptrfullname = $row['fullname'];
            $ptremail = $row['email'];
    
    ?>
      <tr>
        <th scope="row"><?php echo $ptrid ?></th>
        <td><?php echo $ptrfullname?></td>
        <td><?php echo $ptremail ?></td>
        <td><?php echo "<a href='edit.php?id_admin=$ptrid'>UPDATE</a> | 
								<a href='admin_delete.php?id_admin=$ptrid'>DELETE</a>"; ?> </td>
      </tr>
    </tbody>
        <?php
    }
